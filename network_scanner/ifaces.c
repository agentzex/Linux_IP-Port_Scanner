...
void *start_sniffer(void *args)
{
   ...
   sprintf(interface_name,"%s", datos->interface);
                    ...
                    const u_char* packet)
{
   ...
   if (memcmp(new_header->smac, smac, 6) != 0) {
      ...
      if (memcmp(type, ARP_REPLY, 2) == 0) {
         ...
         add_to_ip_list(new_reg->sip, new_reg->vendor, interface_name);
...
int inject_init(char const *disp)
{
   char *ourmac;
...
void forge_arp(char *source_ip, char *dest_ip)
{
   in_addr_t sip, dip;
